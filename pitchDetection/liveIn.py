...
def hz_to_note(hz):
    """Converts a frequency in Hertz to the corresponding musical note."""
    # Define the reference frequency and note names
    ref_freq = 440  # A4 note's frequency in Hertz
    notes = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B']

    # Calculate the index of the note in the notes list
    index = round(12 * math.log2(hz / ref_freq)) % 12

    # Calculate the octave number
    octave = round((12 * math.log2(hz / ref_freq)) / 12) + 4
...
def detect_pitch(int_data):
	...
	if len(dips) > 1:
		...
		print('\r'+hz_to_note(int(detect_pitch.avg)), end='')
